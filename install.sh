...
APP_NAME="appstore"
# Minimal env file holding ONLY the variables Compose interpolates (DB_PASSWORD).
# We never let Compose auto-load ./.env, because Compose interpolates env files
# and would mangle the bcrypt hash ($2b$...) in .env and print noisy
# "variable is not set" warnings. The app reads the full ./.env via a mount.
...
# docker compose wrapper: runs from INSTALL_DIR with our dedicated env file.
dc() { ( cd "$INSTALL_DIR" && docker compose --env-file "$COMPOSE_ENV_FILE" "$@" ); }
...
EOF
  ...
  # compose.env: escape any '$' so Compose interpolation yields the literal
  # password (no-op for the default hex password).
  printf 'DB_PASSWORD=%s\n' "${DB_PASSWORD//\$/\$\$}" > "$COMPOSE_ENV_FILE"
  chmod 600 "$COMPOSE_ENV_FILE"

  c_info "Building the web image (used to hash the admin password securely)…"
  dc build web >/dev/null

  c_info "Hashing admin password with bcrypt…"
  hash="$(dc run --rm --no-deps \
...
start_stack() {
  ...
  dc up -d --build db redis web bot

  c_info "Waiting for the database to become healthy…"
  local tries=0
  until dc exec -T db pg_isready -U appstore >/dev/null 2>&1; do
    tries=$((tries + 1)); [ "$tries" -gt 30 ] && die "Database did not become ready."
    sleep 2
  done

  # Reconcile the DB user's password with the current .env. The Postgres
  # password is fixed at first volume init and can otherwise diverge from .env
  # across re-installs, causing "password authentication failed". We connect
  # over the container's local socket (trusted, no password) and ALTER the
  # password to match — non-destructive, no data loss, idempotent.
  c_info "Synchronizing database credentials…"
  local pw_sql
  pw_sql="${DB_PASSWORD//\'/\'\'}"  # escape single quotes for SQL
  if dc exec -T db psql -U appstore -d appstore -v ON_ERROR_STOP=1 \
       -c "ALTER USER appstore WITH PASSWORD '${pw_sql}';" >/dev/null 2>&1; then
    c_ok "Database credentials synchronized."
  else
    c_warn "Could not pre-sync DB credentials; relying on the existing password."
  fi

  c_info "Running database migrations…"
  if ! dc run --rm web alembic upgrade head; then
    c_err "Migrations failed."
    c_err "If this is 'password authentication failed for user \"appstore\"', the"
    c_err "Postgres data volume is in an unexpected state. With no real data yet"
    c_err "(Phase 1) you can reset it safely:"
    c_err "    cd $INSTALL_DIR && docker compose --env-file compose.env down -v && bash install.sh"
    die "Aborting after migration failure."
  fi

  c_info "Creating the main admin user…"
  dc run --rm web python -m scripts.create_admin
